    ...
    <>
      ...
      <div className="w-full mb-16 min-h-[90vh] relative">
        <div className="relative overflow-hidden h-[90vh]">
              ...
              {featuredArticles.map((article, index) => (
                <Link key={article.id} href={`/blogs/${article.slug}`} className="w-full flex-shrink-0 h-full">
                  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-y-8 lg:gap-x-20 pt-2 pb-6 lg:pt-4 lg:pb-8 px-8 lg:px-12 cursor-pointer hover:bg-white/10 transition-colors h-full items-center backdrop-blur-sm">
                      {/* Article Content */}
                      <div className="space-y-6">
                        <h2 className="text-sm font-medium text-gray-600 uppercase tracking-wide mb-1">
                          Featured Article
                        </h2>
                        <h1 className="text-4xl lg:text-5xl font-bold text-black leading-tight">{article.title}</h1>

                        <div className="flex flex-wrap gap-2">
                          {article.categories.map((category) => (
                            <span
                              key={category}
                              className="px-3 py-1 border border-gray-300 rounded-full text-sm font-medium text-gray-700 bg-white/80"
                            >
                              {category}
                            </span>
                          ))}
                      ...
                      {/* Article Image */}
                      <div className="relative h-72 lg:h-[500px] rounded-lg overflow-hidden">
                        <Image
          ...
          <button
            onClick={prevSlide}
            className="absolute left-8 lg:left-16 top-1/2 transform -translate-y-1/2 p-3 rounded-full bg-white/90 shadow-lg hover:bg-white transition-colors z-20"
          >
            <ChevronLeft className="w-6 h-6 text-gray-600" />
          </button>
          <button
            onClick={nextSlide}
            className="absolute right-8 lg:right-16 top-1/2 transform -translate-y-1/2 p-3 rounded-full bg-white/90 shadow-lg hover:bg-white transition-colors z-20"
          >
